...
public class ColorDialog extends JDialog {
	...
	private JPanel createCMYKPanel(ColorDialogResult result, int imageWidths) {	
		...
		ColorSlider csCyan = new ColorSlider("C:", Math.round(result.getPixel().getCyan()*255), cmykMediator.getCyanImage());
		ColorSlider csMagenta = new ColorSlider("M:", Math.round(result.getPixel().getMagenta()*255), cmykMediator.getMagentaImage());
		ColorSlider csYellow = new ColorSlider("Y:", Math.round(result.getPixel().getYellow()*255), cmykMediator.getYellowImage());
		ColorSlider csBlack = new ColorSlider("K:", Math.round(result.getPixel().getBlack()*255), cmykMediator.getBlackImage());
	...
	private JPanel createYCbCrPanel(ColorDialogResult result, int imageWidths) {	
		...
		ColorSlider csCb = new ColorSlider("Cb:", result.getPixel().getCb(), ycbcrMediator.getCbImage());
		ColorSlider csCr = new ColorSlider("Cr:", result.getPixel().getCr(), ycbcrMediator.getCrImage());
	...
	private JPanel createHSVPanel(ColorDialogResult result, int imageWidths) {	
		...
		ColorSlider csHue = new ColorSlider("H:", Math.round(result.getPixel().getHue()*255/360), hsvMediator.getHueImage());
		ColorSlider csSaturation = new ColorSlider("S:", Math.round(result.getPixel().getSaturation()*255), hsvMediator.getSaturationImage());
		ColorSlider csValue = new ColorSlider("V:", Math.round(result.getPixel().getValue()*255), hsvMediator.getValueImage());
