 ...
 */
public class MirrorStrategy extends PaddingStrategy {
	/**
	...
	public Pixel pixelAt(ImageX image, int x, int y) {
		...
		if (x >= image.getImageWidth()) {
			x = image.getImageWidth() - (x % image.getImageWidth());
		}
        if (y >= image.getImageHeight()) {
            y = image.getImageHeight() - (y % image.getImageHeight());
        }
	...
	public PixelDouble pixelAt(ImageDouble image, int x, int y) {
		...
		if (x >= image.getImageWidth()) {
			x = image.getImageWidth() - (x % image.getImageWidth()) - 1;
		}
        if (y >= image.getImageHeight()) {
            y = image.getImageHeight() - (y % image.getImageHeight()) - 1;
        }
        return image.getPixel(x, y);
	}
}
